...
install_dependencies() {
    ...
    if [[ "$OS" == *"Ubuntu"* ]] || [[ "$OS" == *"Debian"* ]]; then
        apt-get update
        apt-get install -y python3 python3-pip iptables fail2ban curl wget net-tools dnsutils psutil setproctitle
    elif [[ "$OS" == *"CentOS"* ]] || [[ "$OS" == *"Red Hat"* ]] || [[ "$OS" == *"Fedora"* ]]; then
        yum update -y
        yum install -y python3 python3-pip iptables fail2ban curl wget net-tools bind-utils psutil
...
install_files() {
    ...
    cp -r ../astracat_guard/* "$INSTALL_DIR/"
...
User=root
Group=root
ExecStart=/usr/bin/python3 /opt/astracat_guard/lib/optimized_guard_daemon.py
Restart=always
RestartSec=10
